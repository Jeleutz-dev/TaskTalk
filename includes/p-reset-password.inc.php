<?php
    ...
    if (isset($_POST["reset-password-submit"])) 
    {
     ...
     if (empty($password) || empty($passwordRepeat)) {
          header("Location: p-reset-password.php?newpwd=empty");
          exit();
     } else if ($password != $passwordRepeat) {
          header("Location: p-reset-password.php?newpwd=pwdnotsame");
          exit();
     }
     ...
     if (!mysqli_stmt_prepare($stmt, $sql)) {
       ...
       if (!$row = mysqli_fetch_assoc($result)) {
            ...
            if  ($tokenCheck === false) {
                ...
                $sql = "SELECT * FROM faculty WHERE emailUsers=?;";
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                  echo "There was an error!";
                  exit();
                ...
                if (!$row = mysqli_fetch_assoc($result)) {
                    ...
                    $sql = "UPDATE faculty SET pwdUsers=? WHERE emailUsers=?";
                    $stmt = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($stmt, $sql)) {
                      echo "There was an error!";
                      exit();
                      ...
                      if (!mysqli_stmt_prepare($stmt, $sql)) {
                        ...
                        mysqli_stmt_bind_param($stmt, "s", $tokenEmail);
                        mysqli_stmt_execute($stmt);
                        header("Location: ../p-login.php?newpwd=passwordupdated");
                    }
                    }
                }
            }
