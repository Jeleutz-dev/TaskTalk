...
if (isset($_POST['Add-Task']))
{
    ...
    else
    {
        ...
        else
        {
            ...
            $qry = mysqli_query($conn,"SELECT * FROM groups where projid = '".$projid."'");
            $grp = mysqli_fetch_assoc($qry);
            $grpno = $grp['groupno'];
            $grpid = $grp['idGroups'];
            
            mysqli_stmt_bind_param($stmt, "ssssssssssssss", $course, $section, $subject, $projid, $grpid, $grpno, $createdby, $taskmember, $taskname, $taskdesc, $taskmember, $tend,
                    $tduration,
                    $taskstat);
            ...
            header("Location: ../Professors/project-board.php?course=$course&section=$section&subject=$subject&id=$projid&projname=$projname&add_task=success");
            exit();
            }
...
{
    header("Location: ../Professors/project-board.php?course=$course&section=$section&subject=$subject&id=$projid&projname=$projname&grpno=$grpno&grpid=$grpid&leader=$leader");
    exit();
}