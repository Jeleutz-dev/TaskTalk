                    ...
                    <div class="col-lg-12">
                        <div class="mb-4" >
                            <div class="section-title">
                                ...
                                <?php
                                    $query2 = mysqli_query($conn, "SELECT * FROM projects where id = '".$_GET['id']."'");
                                    $query2 = mysqli_fetch_assoc($query2);
                                    if($_GET['check'] == 'Individual'){
                                ?>
                                    <p>Individual Grading <i>Total of <?php echo $query2['score'];?> points</i></p>
                                <?php
                                    }
                                    elseif($_GET['check'] == 'Group'){
                                ?>
                                    <p>By Group Grading <i>Total of <?php echo $query2['score'];?> points</i></p>
                                <?php
                                    }
                                    else{
                                ?>
                                    <p>Individual and Group Grading <i>Total of <?php echo $query2['score'];?> points</i></p>
                                <?php
                                    }
                                ?>
                            ...
                            <?php
                                if($_GET['check'] == 'Individual'){
                            ?>
                                <form action="../includes/p-individual.inc.php" method="post">
                                    <?php
                                        $leadname = mysqli_query($conn, "SELECT * FROM user WHERE uidUsers='".$_GET['leader']."'");
                                        $leader = mysqli_fetch_assoc($leadname);
                                    ?>

                                    <div>
                                        <div>
                                            <span>Group Leader:&nbsp;&nbsp;&nbsp;<?php echo $leader['f_name']. " " .$leader['l_name']?></span>
                                        </div>
                                        <input type="number"  class="form-control" style="" id ="gradelead" name="gradelead" placeholder="Input Final Grade" required>
                                    </div>
                                    <br>

                                    <?php
                                        $members = mysqli_query($conn,"SELECT * FROM groups WHERE projid='".$_GET['id']."' AND idGroups='".$_GET['grpid']."' AND groupno='".$_GET['grpno']."'");
                                        $x=0;
                                        while($mem = mysqli_fetch_assoc($members)){
                                            $list = explode(",",$mem['members']);
                                            foreach($list as $grpmem){
                                                $x++;
                                                $qname = mysqli_query($conn, "SELECT * FROM user WHERE uidUsers='".$grpmem."'");
                                                $flname = mysqli_fetch_assoc($qname);
                                    ?>

                                    <div>
                                        <div>
                                            <span>Group Member:&nbsp;&nbsp;&nbsp;<?php echo $flname['f_name']. " " .$flname['l_name']?></span>
                                        </div>
                                        <input type="number"  class="form-control" style="" id ="grade[<?php echo $x?>]" name="grade[<?php echo $x?>]" placeholder="Input Final Grade" required>
                                    </div>
                                    <br>
                                    ...
                                    <input type="hidden" class="form-control" id="uid" name="uid" value="<?php  echo $_SESSION['userUid']; ?>">
                                    <input type="hidden" class="form-control" name="id" value="<?php  echo $_GET['id']; ?>">
                                    <input type="hidden" id="action" name="action" value="Upload Grade" readonly>
                                    <input type="hidden" class="card-title"name="gradetype" value="<?php echo $_GET['check']; ?>">
                                    <span></span>
                                    <?php
                                        $query1 = mysqli_query($conn, "SELECT * FROM projsubmit where projid = '".$_GET['id']."'");
                                        $query = mysqli_fetch_assoc($query1);
                                        $query2 = mysqli_query($conn, "SELECT * FROM projects where id = '".$_GET['id']."'");
                                        $query2 = mysqli_fetch_assoc($query2);
                                    ?>
                                    <input type="hidden" name="projsubid" id="projsubid" value="<?php  echo $query['id']; ?>" />  
                                    <input type="submit"  class="btn btn-primary"  name="Add-Grade" value="Save">
                                </form>
                            <?php
                                }
                                elseif($_GET['check'] == 'Group'){
                            ?>
                                <form action="../includes/p-grading.inc.php" method='post' id="contact-form" enctype="multipart/form-data">
                                    <!-- <small>Check your students  <a href="subjects.php">progress.</a></small> style="float: right;"-->
                                    <input type="number"  class="" style="" id ="grade" name="grade" placeholder="Input Final Grade" required>
                                    <input type="hidden" class="form-control" id="grpno" name="grpno" value="<?php  echo $_GET['grpno']; ?>">
                                    <input type="hidden" class="form-control" id="grpid" name="grpid" value="<?php  echo $_GET['grpid']; ?>">
                                    <input type="hidden" class="form-control" id="uploadedby" name="uploadedby" value="<?php  echo $_SESSION['userUid']; ?>">
                                    <input type="hidden" class="form-control" name="id" value="<?php  echo $_GET['id']; ?>">
                                    ...
                                    <?php
                                        ...
                                        $query2 = mysqli_fetch_assoc($query2);
                                    ?>
                                    <span> /<?php echo $query2['score'];?></span>
                                    <input type="hidden" name="projsubid" id="projsubid" value="<?php  echo $query['id']; ?>" />  
                                    <input type="submit"  class="btn btn-primary"  name="Add-Grade" value="Save">
                                </form>
                            <?php
                            ...
                            ?>
                                <form action="../includes/p-indivgrp.inc.php" method="post">
                                    <?php
                                        $leadname = mysqli_query($conn, "SELECT * FROM user WHERE uidUsers='".$_GET['leader']."'");
                                        $leader = mysqli_fetch_assoc($leadname);
                                    ?>
                                    ...
                                    ?>
                                    <input type="hidden" name="projsubid" id="projsubid" value="<?php  echo $query['id']; ?>" />  
                                    <input type="number"  class="" style="" id ="grpgrade" name="grpgrade" placeholder="Input Final Grade" required>
                                    <span> /<?php echo $query2['score'];?></span>
                                    <input type="submit"  class="btn btn-primary"  name="Add-Grade" value="Save">
                                </form>
                            <?php
