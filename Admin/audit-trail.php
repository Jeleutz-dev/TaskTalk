...
if (strlen($_SESSION['userUid'])==0) {
    ...
    if(isset($_POST['pdf-submit'])){
        ...
        function generateRow(){
            ...
            while($row = mysqli_fetch_assoc($query)){
                $x++;
                $sql1 = mysqli_query($conn,"SELECT * FROM user where uidUsers = '".$row['uidUsers']."'");
                $q1 = mysqli_fetch_assoc($sql1);
            	$contents .= "
            	<tr>
                    <td>".$x."</td>
                    <td>".($q1['f_name']. " " .$q1['l_name'])."</td>
                    <td>".$row['date']."</td>
                    <td>".$row['action']."</td>
                    <td>".$row['activity_log_id']."</td>
            	</tr>
            	";
            }
    ...
    $pdf->SetTitle("Audit Trail Table");  
    ...
    $content .= '
        ...
        <h4 align="center" style="font: arial;">Generated PDF for TaskTalk System Audit Trail</h4>
        </div>
      	<h4>Audit Trail Table</h4>
    ...
    $pdf->Output('tasktalk-audit-trail.pdf', 'I');
    }
